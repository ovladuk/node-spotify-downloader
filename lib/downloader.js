...
(function() {
  var Downloader, EventEmitter, Logger, Track, async, fs, lodash, spotifyWeb,
      ...
      if (this.config.type === "playlist") {
        ...
        return this.spotify.playlist(this.config.uri, 0, 9001, (function(_this) {
          return function(err, data) {
            ...
            if (_this.config.folder) {
              _this.config.directory = _this.config.directory + "/" + _this.data.name.replace(/\//g, "-") + "/";
            }
        ...
        return this.spotify.get(this.config.uri, (function(_this) {
          return function(err, album) {
            ...
            if (_this.config.folder) {
              _this.config.directory = _this.config.directory + "/" + _this.data.name.replace(/\//g, "-") + (" [" + album.date.year + "]") + "/";
            }
            tracks = [];
        ...
        return this.spotify.library(this.config.username, 0, 9001, (function(_this) {
          return function(err, data) {
            ...
            if (_this.config.folder) {
              _this.config.directory = _this.config.directory + "/Library/";
            }
            _this.data.tracks = lodash.map(data.contents.items, function(track) {
              _this.data.trackCount += 1;
              return track.uri;
